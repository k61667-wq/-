...
1. **대상 사이트 직접 확인 불가.** 현재 Claude Code 원격 실행 환경의 네트워크 정책이 `karrot.evenit.co.kr` 로의 아웃바운드 연결을 차단(프록시 403)하고 있어, 이 문서의 사이트 고유 항목(프레임워크, CSP, 쿠키, API 경로)은 **미확인 상태**다. 아래 분석은 브라우저 표준 동작에 근거한 확정 사항과, 사이트에서 직접 실행해야 확정되는 항목을 구분해 기술한다.
2. 제어 가능 범위는 **"어디서 코드를 실행하느냐"** 로 결정된다. 3개 계층으로 나뉜다.
...
## 4. 체크리스트 (대상 사이트에서 직접 채울 것)

| # | 확인 항목 | 확인 방법 | 결과 |
|---|---|---|---|
| 1 | HTTPS / `isSecureContext` | 프로브 `env.secure` | ☐ |
| 2 | 프레임워크·번들러 | 프로브 `app.globals`, `app.markers`, `net.scripts` | ☐ |
| 3 | 헤더 CSP 존재·`script-src` 정책 | 프로브 `net.fetchSame` 또는 러너 `responseHeaders` | ☐ |
| 4 | 인라인 스크립트 주입 가능 여부 | 프로브 `csp.inlineScript` | ☐ |
| 5 | `X-Frame-Options` / `frame-ancestors` | 러너 헤더, 하네스 iframe 테스트 | ☐ |
| 6 | 인증 방식(HttpOnly 쿠키 / localStorage 토큰) | 러너 `cookies[].httpOnly`, 프로브 `storage.ls` 키 수 | ☐ |
| 7 | 사이트 내부 API 경로·호스트 | 러너 `requestsByHost`, 프로브 `net.resources` | ☐ |
| 8 | 외부 API CORS 허용 | `CONFIG.crossOriginTestUrl` 설정 후 `net.fetchCross` | ☐ |
| 9 | Service Worker 등록 여부 | 프로브 `worker.sw` | ☐ |
| 10 | 교차 출처 iframe 포함 여부·접근성 | 프로브 `dom.iframes` | ☐ |
| 11 | 클립보드·다운로드·인쇄 API 가용성 | 프로브 `gesture.*` | ☐ |
| 12 | 권한 상태(알림·위치·클립보드) | 프로브 `perm.*` | ☐ |
| 13 | 콘솔 오류·실패 요청 수 | 러너 `consoleErrorCount`, `requestFailures` | ☐ |
| 14 | 폼 전송 대상(action 호스트) | 프로브 `app.forms` | ☐ |
| 15 | 로그인 후 추가로 열리는 API·저장소 | 러너 `--storage-state` 재실행 후 1~14 비교 | ☐ |
...
| 구분 | 내용 | 대응 |
|---|---|---|
| 미확인 | 대상 사이트 실측 없이 작성됨. 프레임워크·CSP·인증 방식은 체크리스트로 확정 필요 | 4장 체크리스트 실행 후 본 문서 갱신 |
...
1. 사용자 PC 에서 `node tools/run_probe.mjs https://karrot.evenit.co.kr/ --out reports/karrot` 실행 후 `report.json` 을 공유 → 4장 체크리스트 확정 및 문서 2장 사이트 고유 열 갱신.
2. 로그인이 필요한 사이트라면 `--headed --save-state auth.json` 으로 인증 상태를 저장하고 재실행해 인증 후 범위를 별도 확인.
